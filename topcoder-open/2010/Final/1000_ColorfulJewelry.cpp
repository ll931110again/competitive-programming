...
struct ColorfulJewelry {
  ...
  int getChains(vector<string> s, int k) {
    ...
    for (int i = 1; i < m; i++)
      for (int j = 0; j < n; j++) {
        numRed[i][j] += numRed[i - 1][j];
        numBlue[i][j] += numBlue[i - 1][j];
        numGreen[i][j] += numGreen[i - 1][j];
      }

    for (int j = 1; j < n; j++)
      for (int i = 0; i < m; i++) {
        numRed[i][j] += numRed[i][j - 1];
        numBlue[i][j] += numBlue[i][j - 1];
        numGreen[i][j] += numGreen[i][j - 1];
      }
    ...
    for (int i = 0; i <= m * n; i++) {
      ...
      for (int j = 1; i + j <= m * n; j++)
        sumDelta[i][j] = (sumDelta[i][j - 1] + c[i + j][j]) % mod;
    }
    ...
    for (int i = m * n; i >= 0; i--)
      for (int j = m * n - i; j >= 0; j--)
        if (maxGreen[i][j] >= 0) {
          ...
          if (i)
            maxGreen[i - 1][j] = max(maxGreen[i - 1][j], maxGreen[i][j]);
          if (j)
            maxGreen[i][j - 1] = max(maxGreen[i][j - 1], maxGreen[i][j]);
          if (!i && !j)
            ret = (ret + mod - 1) % mod; // No jewels
        }
  ...
  void adjust(int xa, int ya, int xb, int yb, int delta) {
    ...
    if (xa) {
      Red -= numRed[xa - 1][yb];
      Blue -= numBlue[xa - 1][yb];
      Green -= numGreen[xa - 1][yb];
    }
    if (ya) {
      Red -= numRed[xb][ya - 1];
      Blue -= numBlue[xb][ya - 1];
      Green -= numGreen[xb][ya - 1];
    }
    if (xa && ya) {
      Red += numRed[xa - 1][ya - 1];
      Blue += numBlue[xa - 1][ya - 1];
      Green += numGreen[xa - 1][ya - 1];
    }
    totalRed += Red * delta;
    totalGreen += Green * delta;
    totalBlue += Blue * delta;
  }
