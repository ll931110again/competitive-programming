...
class StRings {
  ...
  static bool canPut(const vector<int> &S, int N, int pos, int cand) {
    ...
    while (true) {
      ...
      ++edges;
      cur = nx;
      if (cur == u) return edges >= N;
      if (edges > N) return true;
    }
  }
 ...
 public:
  vector<int> getSmallest(int N, vector<int> first) {
    ...
    if (!partialValid(S, N)) return {};

    for (int pos = L; pos < N; ++pos) {
      bool placed = false;
      for (int cand = 1; cand <= N; ++cand) {
        if (used[cand]) continue;
        if (!canPut(S, N, pos, cand)) continue;
        S[pos] = cand;
        used[cand] = 1;
        placed = true;
        break;
    ...
    if (N > 50) return vector<int>(S.end() - 50, S.end());
    return S;
  }
};
