...
static long long minInitialForTasks(vector<Task>& tasks) {
    ...
    // the minimal starting value is minimized by sorting by (req - cost) descending.
...
struct DpRes {
    long long need;  // minimal soldiers needed upon arriving (before attacking this castle)
    long long loss;  // total soldiers removed in this subtree (deaths+garrisons)
};

static DpRes dfsSolve(int u, int parent, const vector<Castle>& castles, const vector<vector<int>>& adj) {
    vector<Task> tasks;
    long long lossSum = 0;
