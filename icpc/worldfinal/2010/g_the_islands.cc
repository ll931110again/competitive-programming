...
int main() {
    ...
    while (cin >> n >> b1 >> b2) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) dp[i][j].fill(INF);
        }
            ...
            for (int a = 0; a <= k; a++) {
                for (int b = 0; b <= k; b++) {
                    ...
                    for (int m = 0; m < 4; m++) {
                        ...
                        {
                            int nm = m;
                            // If next is a special island, it belongs to return pass, so bits stay 0.
        ...
        for (int k = n - 1; k >= 2; k--) {
            Parent pr = par[a][b][m];
            if (pr.addTo == 'F') addedF.push_back(k);
            else if (pr.addTo == 'R') addedR.push_back(k);
            else break;
            a = pr.pa;
            b = pr.pb;
            m = pr.pm;
        ...
        for (int i = (int)addedF.size() - 1; i >= 0; i--) forward.push_back(addedF[i]);
        vector<int> ret = {0};
        for (int i = (int)addedR.size() - 1; i >= 0; i--) ret.push_back(addedR[i]);

        // ret is 0 -> ... -> bestB in increasing x (forward direction of return-chain).
        // We traverse it backward from bestB back to 0.
        vector<int> tour = forward;
        for (int i = (int)ret.size() - 1; i >= 0; i--) tour.push_back(ret[i]);
