...
static bool connectivityOk(int cur) {
    ...
    while (qs < qe) {
        int u = q[qs++];
        for (int v : nbr[u]) {
            if (seen & (1ULL << v)) continue;
            if (v != cur && (visited & (1ULL << v))) continue;
            seen |= (1ULL << v);
            q[qe++] = v;
            if (!(visited & (1ULL << v))) reachUnvis++;
        }
    }
...
static void dfs(int cur, int step) {
    ...
    for (int i = 0; i < 3; i++) {
        if (step == cpTime[i]) {
            if (cur != cpPos[i]) return;
        } else {
            if (cur == cpPos[i]) return; // visited checkpoint at wrong time
        }
    }

    if (cur == endPos) return; // end must be last

    if (!connectivityOk(cur)) return;

    for (int v : nbr[cur]) {
        if (visited & (1ULL << v)) continue;
        visited |= (1ULL << v);
        dfs(v, step + 1);
        visited ^= (1ULL << v);
    }
...
int main() {
    ...
    while (cin >> m >> n) {
        ...
        for (int i = 0; i < 64; i++) nbr[i].clear();
        for (int r = 0; r < M; r++) {
            for (int c = 0; c < N; c++) {
                int u = id(r, c);
                if (r > 0) nbr[u].push_back(id(r - 1, c));
                if (r + 1 < M) nbr[u].push_back(id(r + 1, c));
                if (c > 0) nbr[u].push_back(id(r, c - 1));
                if (c + 1 < N) nbr[u].push_back(id(r, c + 1));
            }
        }
