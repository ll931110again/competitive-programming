...
We need to decide if an x*y rectangle can be partitioned by repeated straight cuts
along grid lines into n rectangles whose areas match the given parts[].
...
struct Key {
    ...
    int w;
    int h;
    bool operator==(Key const& other) const { return mask == other.mask && w == other.w && h == other.h; }
};
...
static bool solveCase(int x, int y, const vector<int>& parts) {
    ...
    long long total = 0;
    for (int a : parts) total += a;
    if (total != 1LL * x * y) return false;
