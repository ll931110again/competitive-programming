...
// Adapted from the well-known "LineContainer" (KACTL style), but using __int128-safe division.
struct DynamicCHTMax {
  ...
  static long long intersectX(const HullLine &x, const HullLine &y) {
    // Smallest integer x where y(x) >= x(x). Both are linear. Assumes x.m != y.m.
    // Solve y.m*t + y.b >= x.m*t + x.b  =>  (y.b - x.b) >= (x.m - y.m)*t
    // If denom > 0: t <= (y.b - x.b)/denom ; but we want smallest t where y overtakes x
    // KACTL uses: p = floor((b2-b1)/(m1-m2)) for max with increasing slopes, with care.
    // We'll compute as floor((x.b - y.b)/(y.m - x.m)) to match standard form.
    // We want: x.m*t + x.b >= y.m*t + y.b for t <= p ; so p is the last t where x dominates.
    // p = floor((y.b - x.b)/(x.m - y.m)).
  ...
  void addLine(long long m, long long b) {
    ...
    if (x != hull.begin() && isect(--x, y)) isect(x, hull.erase(y));
    while ((y = x) != hull.begin() && (--x)->p >= y->p) isect(x, hull.erase(y));
  }
...
int main() {
  ...
  while (cin >> machineCount >> initialCash >> lastDay) {
    if (machineCount == 0 && initialCash == 0 && lastDay == 0) break;
    ...
    sort(machines.begin(), machines.end(), [](const Machine &a, const Machine &b) {
      if (a.dayAvailable != b.dayAvailable) return a.dayAvailable < b.dayAvailable;
