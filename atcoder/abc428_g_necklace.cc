...
static void dirichlet_conv(const vector<Mint> &a, const vector<Mint> &b,
                           vector<Mint> &c, int U) {
  fill(c.begin(), c.end(), Mint(0));
  for (int i = 1; i <= U; i++) {
    if (!a[i].x) continue;
    for (int j = 1; i * j <= U; j++) c[i * j] += a[i] * b[j];
  }
}
...
static void dirichlet_inv(const vector<Mint> &a, vector<Mint> &b, int U) {
  ...
  for (int i = 1; i <= U; i++) {
    if (!b[i].x) continue;
    for (int j = 2; i * j <= U; j++) b[i * j] -= b[i] * a[j];
  }
}
...
int main() {
  ...
  for (int i = 2; i <= U; i++) Omega[i] = Omega[i / lpf[i]] + 1;
  ...
  int max_omega = 0;
  for (int i = 2; i <= U; i++) max_omega = max(max_omega, Omega[i]);
  ...
  for (int k = 1; k <= max_omega; k++) inv_omega[k] = Mint(k).inv();

  // B = 1 - A(s): B[1]=1, B[n]=-A[n].
  vector<Mint> B(U + 1);
  B[1] = 1;
  for (int i = 2; i <= U; i++) B[i] = Mint(0) - A[i];
  ...
  for (int i = 1; i <= U; i++) G[i] *= Mint(Omega[i]);
  vector<Mint> F(U + 1);
  dirichlet_conv(G, B, F, U);
  for (int i = 2; i <= U; i++) F[i] *= inv_omega[Omega[i]];
