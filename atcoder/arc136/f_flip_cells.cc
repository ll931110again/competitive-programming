...
template <int P> struct ModInt {
  ...
  friend ModInt operator-(ModInt a, ModInt b) { return a -= b; }
  friend ModInt operator*(ModInt a, ModInt b) { return a *= b; }
  friend ModInt operator/(ModInt a, ModInt b) { return a * mod_pow(b, (long long)P - 2); }
...
static void ntt(vector<int> &a, bool invert) {
  ...
  for (int len = 1; len < n; len <<= 1) {
    ...
    if (invert) wlen = mod_pow_int(wlen, MOD - 2);
...
static vector<vector<MI>> mat_mul(const vector<vector<MI>> &a,
                                  const vector<vector<MI>> &b) {
  const int n = (int)a.size();
...
static vector<MI> extend_entry(const vector<Mat> &pw, int from, int to,
                             const vector<MI> &trans, int D) {
  const int L = (int)trans.size();
  vector<MI> row(D);
  for (int t = 0; t < L && t < D; ++t) row[t] = pw[t][to][from];
  for (int t = L; t < D; ++t) {
    MI v(0);
    for (int j = 0; j < L; ++j) v = v + trans[j] * row[t - j - 1];
    row[t] = v;
  }
  return row;
}
...
static MI eval_poly(const vector<MI> &p, MI x) {
  MI r(0);
  for (int i = (int)p.size() - 1; i >= 0; --i) r = r * x + p[i];
  return r;
}
...
static vector<MI> deriv_poly(const vector<MI> &p) {
  ...
  for (int i = 1; i < (int)p.size(); ++i) q[i - 1] = p[i] * MI(i);
  return q;
}
...
static MI expected_from_recurrence(const vector<MI> &f, const vector<MI> &trans) {
  ...
  vector<MI> den(L + 1);
  den[0] = MI(1);
  for (int i = 0; i < L; ++i) den[i + 1] = MI(0) - trans[i];

  vector<MI> num(L);
  for (int i = 0; i < L; ++i)
    for (int j = 0; j <= i; ++j) num[i] = num[i] + f[j] * den[i - j];

  const MI Q1 = eval_poly(den, MI(1));
  if (Q1 == MI(0)) return MI(0);
...
int main() {
  ...
  Mat T(n, vector<MI>(n));
  for (int j = 0; j <= W; ++j) {
    if (j > 0) T[j - 1][j] = MI(j) / MI(W);
    if (j < W) T[j + 1][j] = MI(W - j) / MI(W);
  }

  const int D = 2 * H * W + 50;
  vector<Mat> pw(n, Mat(n, vector<MI>(n)));
  for (int i = 0; i < n; ++i) pw[0][i][i] = MI(1);
  for (int t = 1; t < n; ++t) pw[t] = mat_mul(pw[t - 1], T);
  ...
  for (int i = 1; i <= D; ++i) fact[i] = fact[i - 1] * MI(i);
  inv_fact[D] = mod_pow(fact[D], MOD - 2);
  for (int i = D; i >= 1; --i) inv_fact[i - 1] = inv_fact[i] * MI(i);
  ...
  auto build_g = [&](bool from_target) {
    ...
    for (int ri = 0; ri < H; ++ri) {
      ...
      vector<int> coeff(D);
      for (int t = 0; t < D; ++t) coeff[t] = (row[t] * inv_fact[t]).x;
  ...
  for (int i = 0; i < D; ++i) {
    f[i] = gg[i];
    for (int j = 1; j < i; ++j) f[i] = f[i] - f[j] * hh[i - j];
  }
