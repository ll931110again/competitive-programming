...
int tree_count;
i64 fenwick[k_max_trees];

void fenwick_add(int index, i64 value) {
  for (; index <= tree_count; index += index & -index) {
    fenwick[index] += value;
  }
}

i64 fenwick_prefix(int index) {
  i64 sum = 0;
...
void fenwick_range_add(int left, int right, i64 value) {
  if (left > right) {
    return;
  }
...
i64 height_at(int index) {
  return fenwick_prefix(index);
}

int first_at_least(i64 value) {
  int low = 1;
...
void fertilize(int count, i64 min_height) {
  ...
  count = right - left + 1;
  i64 top_height = height_at(right);
...
int count_range(i64 low, i64 high) {
...
int main() {
  ...
  vector<pair<i64, int>> trees(tree_count);
  ...
  for (int i = 1; i <= tree_count; i++) {
    i64 diff = trees[i - 1].first - (i == 1 ? 0 : trees[i - 2].first);
    fenwick_add(i, diff);
  }
