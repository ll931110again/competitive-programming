...
static void ntt(vector<int64> &a, bool invert, int64 mod, int64 root) {
    ...
    if (invert) {
        ...
        for (int64 &x : a) x = (i128)x * inv_n % mod;
    }
}
...
static vector<int64> convolution_ll(const vector<int64> &aa, const vector<int64> &bb) {
    ...
    for (int i = 0; i < (int)aa.size(); ++i) fa[i] = aa[i] % MOD1;
    for (int i = 0; i < (int)bb.size(); ++i) fb[i] = bb[i] % MOD1;
    ntt(fa, false, MOD1, ROOT1);
    ntt(fb, false, MOD1, ROOT1);
    for (int i = 0; i < n; ++i) fa[i] = (i128)fa[i] * fb[i] % MOD1;
    ntt(fa, true, MOD1, ROOT1);

    vector<int64> ga(n), gb(n);
    for (int i = 0; i < (int)aa.size(); ++i) ga[i] = aa[i] % MOD2;
    for (int i = 0; i < (int)bb.size(); ++i) gb[i] = bb[i] % MOD2;
    ntt(ga, false, MOD2, ROOT2);
    ntt(gb, false, MOD2, ROOT2);
    for (int i = 0; i < n; ++i) ga[i] = (i128)ga[i] * gb[i] % MOD2;
...
static int64 solve_one(int64 P, int64 L, int64 N, int64 A1, int64 A2, int64 A3, int64 A4, int64 A5,
    ...
    int64 zpairs =
        ca[0] * M + cb[0] * N - ca[0] * cb[0];
    int64 ans = 0;
    if (L > 0) ans += zpairs;

    int64 Llen = P - 1;
    if (Llen <= 0) return ans;
