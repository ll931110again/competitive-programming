...
static void dijkstra(int V, const vector<vector<Edge>>& g, vector<Best>& dist) {
    ...
    for (int v = 0; v < V; v++) {
        if (dist[v].cost < INF_COST) pq.push(Node{dist[v].cost, dist[v].cnt, v});
    }
    ...
    while (!pq.empty()) {
        ...
        int v = cur.v;
        if (vis[v]) continue;
        if (cur.cost != dist[v].cost || cur.cnt != dist[v].cnt) continue;
        vis[v] = 1;
    ...
    for (int tc = 1; tc <= T; tc++) {
        ...
        for (int mask = 1; mask <= FULL; mask++) {
            ...
            for (int sub = (mask - 1) & mask; sub; sub = (sub - 1) & mask) {
                int other = mask ^ sub;
                if (sub > other) continue; // avoid doing both (sub,other) and (other,sub)
                ...
                for (int v = 1; v < V; v += 2) {
                    ...
                    if (a.cost >= INF_COST || b.cost >= INF_COST) continue;
                    int orig = v / 2;
                    Best comb{a.cost + b.cost - (long long)food[orig], a.cnt + b.cnt - 1};
                    if (better(comb, dp[mask][v])) dp[mask][v] = comb;
                }
            }
        ...
        for (int v = 1; v < V; v += 2) {
            if (better(dp[FULL][v], ans)) ans = dp[FULL][v];
        }
