...
static inline long long valueAt(int x, int y, int z, const vector<Bomb>& bombs) {
    ...
    for (const auto& b : bombs) best = min(best, dist2(x, y, z, b));
    return best;
}
...
static long long solveCase(const vector<Bomb>& bombs) {
                ...
                for (int dx : {-step, 0, step}) {
                    for (int dy : {-step, 0, step}) {
                        for (int dz : {-step, 0, step}) {
                            ...
                            if (nx < 0 || nx > 1000 || ny < 0 || ny > 1000 || nz < 0 || nz > 1000) continue;
    ...
    long long best = 0;
    // corners + center
    for (int cx : {0, 1000}) for (int cy : {0, 1000}) for (int cz : {0, 1000})
    ...
    while (!pq.empty()) {
        ...
        if (lx >= ly && lx >= lz && lx > 0) {
            ...
            if (a.ub > best) pq.push(a);
            if (b.ub > best) pq.push(b);
