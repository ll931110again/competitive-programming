...
def game_value(n, k, s):
    ...
    def dp(state, rounds_left):
        if rounds_left == 0:
            # red score = potatoes on odd indices (Blue positions) -> Red's score
        ...
        best = -1e9  # Red maximizes red score (potatoes on Blue positions)
        from itertools import product
        ...
        for rch in red_choices:
            ...
            if min_val > best:
                best = min_val
        return best
    red_score = dp(init, k)  # this is Red's score = potatoes on Blue positions
