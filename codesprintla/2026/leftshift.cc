...
static int longest_even_manacher(int n, F&& getch, const vector<int>& suffix_even_ub,
  ...
  for (int i = 0; i < n; i++) {
    if (suffix_even_ub[(size_t)i] <= best_here)
      break;

    int k = (i > r) ? 0 : min(d2[(size_t)(l + r - i + 1)], r - i + 1);
    while (i + k < n && i - k - 1 >= 0 && getch(i + k) == getch(i - k - 1)) {
      ++k;
    }
    d2[(size_t)i] = k;
    if (2 * k > best_here)
...
int main() {
  ...
  int best = longest_even_manacher(
      n, [&](int p) -> char { return S[(size_t)p]; }, suffix_even_ub, d2_buf, 0);
  ...
  for (int ls = 0; ls < n; ++ls) {
    if (suffix_even_ub[0] <= best)
      break;
    for (int rs = ls + 1; rs < n; ++rs) {
      const int prev = best;
      const int v = longest_even_manacher(
          n, [&](int p) -> char { return char_after_shift(S, ls, rs, p); }, suffix_even_ub, d2_buf,
          best);
      if (v > prev) {
        best = v;
        out_L = ls;
        out_R = rs;
        if (best == n)
          break; // can't do better than full length
      }
    }
    if (best == n)
      break;
