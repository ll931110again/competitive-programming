...
static int solve_case(const string& original) {
  ...
  int pat_id[3][3];
  for (int a = 0; a < 3; a++)
    for (int b = 0; b < 3; b++) pat_id[a][b] = -1;
  for (int p = 0; p < 6; p++) pat_id[pat[p][0]][pat[p][1]] = p;
  ...
  auto close_pops = [&]() {
    ...
    for (int p = 0; p < 6; p++) {
      for (int h = n; h >= 3; h--) {
        int v = dp2[p][h];
        if (v < INF && v + 1 < dp2[p][h - 1]) dp2[p][h - 1] = v + 1;
      }
      if (dp2[p][2] < INF) {
    ...
    for (int c = 0; c < 3; c++) {
      if (dp1[c] < INF) dp0 = min(dp0, dp1[c] + 1);
    }
  };
  ...
  for (int i = 0; i < n; i++) {
    ...
    // Print if possible.
    if (dp1[need] < INF) ndp1[need] = min(ndp1[need], dp1[need] + 1);
    for (int p = 0; p < 6; p++) {
      for (int h = 2; h <= n; h++) {
        int v = dp2[p][h];
        if (v >= INF) continue;
    ...
    if (dp0 < INF) ndp1[need] = min(ndp1[need], dp0 + 2);

    for (int top = 0; top < 3; top++) {
      if (dp1[top] >= INF) continue;
      if (top == need) continue;  // never push same as top
