 ...
 *
 * We compute the shortest path from 1 to each node u, and the shortest path
 * from each node u to n.
 *
 * Then we compute the lexicographically minimal path stepwise. In each step, we
 * find out the minimum color that could result to a valid path. An edge u -> v
 * of color c is valid if v belongs to a shortest path from 1 to n. To check
