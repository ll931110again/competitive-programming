...
int rec(int a,int b)
{
    ...
    if (f[a][b] > tmp)
    {
        f[a][b] = tmp;  trace[a][b] = make_pair(a,b);
    }
    if (b < n)
    {
        ...
        if (f[a][b] > tmp)
        {
            f[a][b] = tmp;  trace[a][b] = make_pair(a,b + 1);
        }
    }
    if (b < n)
    {
        ...
        if (f[a][b] > tmp)
        {
            f[a][b] = tmp;  trace[a][b] = make_pair(b,b + 1);
        }
    }
    return f[a][b];
}
...
void track(int a,int b)
{
    ...
    printf("%d %d\n", u, v);
    if (u == a && v == b) track(b + 1,b + 2);
    else if (u == a && v == b + 1) track(b,b + 2);
    else if (u == b && v == b + 1) track(a,b + 2);
}
