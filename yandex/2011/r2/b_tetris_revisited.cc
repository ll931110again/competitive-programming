// Codeforces 86 (Yandex.Algorithm 2011: Round 2) — B. Tetris revisited
...
bool isolate(int fx, int fy) {
  if (outside(fx, fy) || ans[fx][fy] >= 0 || s[fx][fy] == '#')
    return false;
  for (int i = 0; i < 4; i++) {
    int tx = fx + dx[i], ty = fy + dy[i];
    if (!outside(tx, ty) && ans[tx][ty] < 0 && s[tx][ty] != '#')
      return false;
  }
  return true;
}
...
void cover(vector<pair<int, int>> p) {
  ...
  for (int i = 0; i < p.size(); i++)
    ans[p[i].first][p[i].second] = -1;
  memset(ok, true, sizeof(ok));
  for (int i = 0; i < p.size(); i++)
    for (int j = 0; j < 4; j++) {
      int sx = p[i].first + dx[j], sy = p[i].second + dy[j];
      if (outside(sx, sy) || ans[sx][sy] < 0)
        continue;
      ok[ans[sx][sy]] = false;
    }
  for (int i = 0; i < 10; i++)
    if (ok[i]) {
      for (int j = 0; j < p.size(); j++) {
        ans[p[j].first][p[j].second] = i;
        ret[p[j].first][p[j].second] = i + '0';
      }
      return;
    }
}
