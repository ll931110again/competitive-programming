#!/usr/bin/env python3
"""Download latest accepted Codeforces submissions for Yandex.Algorithm 2011.

Rounds (Codeforces gym contests):
  quali1 — contest 81
  quali2 — contest 82
  r1     — contest 85
  r2     — contest 86
...
Sources require either:
  - codeforces/.cf_api.env (CODEFORCES_API_KEY + CODEFORCES_API_SECRET), or
  - codeforces/.cf_cookie or CODEFORCES_COOKIE (browser Cookie header).

Usage::

    python3 yandex/2011/download_submissions.py
    python3 yandex/2011/download_submissions.py --handle ll931110
"""
...
def latest_ok_by_index(handle: str, contest_id: int) -> tuple[str, list[dict], dict[str, dict]]:
    ...
    return contest_name, problems, best
...
def write_submission(
    ...
    file_ext = ext or extension_for_language(sub.get("programmingLanguage", ""))
    fn = f"{prefix}_{slug}.{file_ext}"
...
def download_round(
    ...
    contest_name, _problems, best = latest_ok_by_index(handle, contest_id)
    index_to_title = {p["index"]: p["name"] for p in _problems}
    if not best:
        print(f"{folder}: no accepted submissions for {handle}")
        write_sources_manifest(out_dir, contest_id, contest_name, index_to_title, best)
        return 0

    manifest = write_sources_manifest(
        out_dir, contest_id, contest_name, index_to_title, best
    )
    print(f"{folder}: {len(best)} problem(s), manifest {manifest}")
    ...
    if not cookie:
        print(
            f"  Skipped source download (set {DEFAULT_API_ENV} or {DEFAULT_COOKIE_FILE})",
            file=sys.stderr,
        )
        return 0
    ...
    for idx in sorted(best, key=lambda x: (len(x), x)):
        ...
        if "Please wait" in page or "being checked" in page:
            print(f"  {idx}: Cloudflare challenge for {url}", file=sys.stderr)
            continue
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        help="Parent directory for quali1, quali2, r1, r2, final",
    )
    args = ap.parse_args()
    ...
    if not api and not cookie:
        print(
            ...
            f"  API: copy codeforces/.cf_api.env.example → {args.api_env}\n"
            f"       (keys from https://codeforces.com/settings/api)\n"
            f"  Or:  save browser Cookie header to {DEFAULT_COOKIE_FILE}\n",
            file=sys.stderr,
        )
    ...
    if total == 0 and not api and not cookie:
        return 1
    print(f"Done: wrote {total} source file(s).")
    return 0 if total > 0 or api or cookie else 1
