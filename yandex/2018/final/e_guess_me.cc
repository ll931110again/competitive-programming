...
int main() {
  ...
  for (int it = 0; it < 50; it++) {
     ...
     *
     * If j is a non maximum element, we can detect that if j lies before both j
     * - 1 and j + 1 in the permutation. The probability of this happening is
     * 1/3.
     *
     * After 50 iterations, the probability of not finding the maximum element
     * is (2/3)^50, which is approximately 10^-10. For n = 1000, this is
     * approximately 10^-7, which is negligible.
     */
    for (int i = 0; i < n; i++) {
