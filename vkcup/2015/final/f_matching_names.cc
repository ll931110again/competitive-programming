 ...
 *
 * Now process on the tree. At a given node, we'd want to greedily match the red
 * and blue color node at maximum depth as possible. So in a given node, we
 * will:
 *  - First, try to find and match all possible matchings at the subtree level.
 *  - Then, match all the residuals at the node level.
 *
 * To implement this, for a node, we keep track of the red indices and blue
 * indices which have not found their matchings yet. Then we aggregate the info
 * from that node and from its children together, doing small to large merging
